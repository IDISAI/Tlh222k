...
```bash
git clone git@github.com:AIVIETNAM-AIO-HUYTRUONG/tlh222k.git
cd tlh222k
```
...
```bash
git clone https://github.com/AIVIETNAM-AIO-HUYTRUONG/tlh222k.git
cd tlh222k
```
...
tlh222k dùng PostgreSQL 16 chạy trong Docker container:
...
Datasource "db": PostgreSQL database "tlh222k", schema "public" at "localhost:5432"
...
```bash
DATABASE_URL="postgresql://vizteck:vizteck@localhost:5432/tlh222k" pnpm --filter @vizteck/db db:seed
```
...
```powershell
$env:DATABASE_URL = "postgresql://vizteck:vizteck@localhost:5432/tlh222k"
