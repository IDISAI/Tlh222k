...
- Frontend: **Next.js App Router bản mới nhất** (create-next-app chuẩn), TypeScript, Tailwind v4, **shadcn/ui là ưu tiên số 1 cho MỌI UI component** — init shadcn từ Phase 0; sidebar, dialog, dropdown-menu, context-menu, command (⌘K search palette), button, input, tooltip, popover, skeleton… đều lấy từ shadcn; chỉ tự viết markup khi shadcn không có component tương ứng. Icons: lucide-react. Server state: TanStack Query v5. Editor: **BlockNote** (`@blocknote/react` + `@blocknote/mantine`).
- Backend: service per-feature **`apps/svc-notion`** — **Hono** + **`@hono/zod-openapi`** (Swagger tự sinh từ zod schemas tại `/docs` — không bao giờ viết openapi.json tay) + **GraphQL Yoga** (`/graphql`, GraphiQL làm sandbox). **Clean Architecture 4 layers**: `domain/` (entities + repository ports, zero dependency) → `application/` (use-cases, chỉ phụ thuộc domain) → `infrastructure/` (Prisma repositories implement ports) → `interface/` (HTTP routes + GraphQL resolvers); `container.ts` là composition root duy nhất. CORS mở cho các origin frontend.
