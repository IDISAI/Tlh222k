...
function useContentCounts() {
      ...
      .then(([fields, nodes]) => {
        setCounts({
          ...
          roadmaps: nodes.filter(
            (node) => !node.isDeleted && node.nodeType !== "article"
          ).length,
        })
      })
      .catch(() => {
  ...
  useEffect(() => {
    void refresh()
    return subscribeRoadmapUpdates(null, () => void refresh())
  }, [refresh])
...
export function AdminNavigation({
  ...
  const renderItem = ({
    ...
    const active =
      !external && isActive !== false && isCurrentPath(pathname, href)
  ...
  return (
    <nav
      className={cn("mt-5 space-y-4 px-2 text-sm", collapsed && "px-2")}
      aria-label="CMS"
    >
...
export function AdminBreadcrumb() {
  const pathname = usePathname()
  const section = pathname.startsWith(NOTEBOOK_BASE_PATH)
    ? "Notebooks"
    : pathname.startsWith(NOTION_BASE_PATH)
      ? "Tài liệu"
      : pathname.startsWith(`${BASE_PATH}/fields`)
        ? "Lĩnh vực"
        : "Roadmaps"
