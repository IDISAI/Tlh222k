...
export function FieldWorkspace({ id, role }: { id: string; role: CallerRole }) {
  ...
  return <main className="min-h-[calc(100vh-57px)] bg-white text-foreground">
    ...
    <div className="grid min-h-[calc(100vh-115px)] lg:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <section className="relative min-h-[600px] overflow-hidden bg-[radial-gradient(#d7d7d7_1px,transparent_1px)] bg-[size:18px_18px] p-5 lg:p-7"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-bold">Roadmap trong lĩnh vực</h2><p className="text-sm text-muted-foreground">{isNew ? "Lưu lĩnh vực nháp trước khi thêm roadmap." : `${membership.length} roadmap · block có thể thuộc nhiều lĩnh vực.`}</p></div><button type="button" disabled={isNew} onClick={openPicker} className="inline-flex items-center gap-1.5 rounded-lg border bg-white px-3 py-2 text-xs font-semibold shadow-sm hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"><Plus className="size-4" />Thêm roadmap</button></div>
        {membership.length === 0 && <p className="mt-5 rounded-xl border border-dashed bg-white/60 p-5 text-sm text-muted-foreground">Lĩnh vực chưa có roadmap nên người học chưa có điểm đến. Thêm ít nhất một roadmap đã xuất bản trước khi xuất bản lĩnh vực.</p>}
