      ...
      <body>
        ...
        <ThemeProvider>
          <div className="flex min-h-screen bg-background">
            <AdminSidebar role={devBypass} />
            <div className="min-w-0 flex-1"><header className="flex h-16 items-center justify-between border-b px-5 lg:px-7"><AdminBreadcrumb /><div className="flex items-center gap-2"><ThemeToggle />{devBypass !== null ? <span className="rounded-full border px-3 py-1 text-xs font-medium text-muted-foreground">{devBypass}</span> : isAuthed && <ClerkLoaded><UserButton /></ClerkLoaded>}</div></header><RoadmapApolloProvider>{children}</RoadmapApolloProvider></div>
          </div>
